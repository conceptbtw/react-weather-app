    ...
    <div className="w-full p-4 md:p-5 border border-white/10 rounded-xl bg-white/10 shadow backdrop-blur">
      ...
      <div className="grid md:grid-cols-3 gap-1">
        <div className="center">
          <img className="max-w-32 md:max-w-40" src={weatherIcon} alt={description} />
        </div>
        <div className="center gap-1">
          <img className="h-6 rounded" src={`https://flagcdn.com/${country.toLowerCase()}.svg`} alt={country} />
          <p className="text-2xl">{name}</p>
        </div>
        <div className="center">
          <DataList>
            <DataItem icon={`${url}thermometer-celsius.svg`} data={`${Math.round(temp)}°`} />
